...
{
    APP_DBG("tuh_mount_cb - USB Host device connected, address: %d", dev_addr);
...
{
    APP_DBG("tuh_umount_cb - USB Host device disconnected, address: %d", dev_addr);
...
void InputState::loop() { 
    ...
    // 处理USB主机任务
    tuh_task(); // 主机模式任务
    // USB_HS_MonitorTinyUSB(); // 监控TinyUSB状态
